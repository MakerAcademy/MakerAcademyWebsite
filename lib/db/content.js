...
export const acceptEdit = async (db, publishedId, draftId) => {
  ...
  const updateContent = db
    ...
    .updateOne(
      { published: cid },
      { $set: { published: did }, $pull: { drafts: { _id: did } } }
    );
  const updatePublished = db
    .collection("documents")
    .updateOne({ _id: draftId }, { $set: { status: "published" } });
  const deleteOld = db.collection("documents").deleteOne({ _id: cid });
};
...
export const rejectEdit = async (db, publishedId, draftId) => {
  ...
  const updateContent = db
    .collection("content")
    .updateOne({ published: cid }, { $pull: { drafts: { _id: did } } });
